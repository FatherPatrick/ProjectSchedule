...
export function AddOnSelector({
  ...
  /** Other active services that can be bundled into the same visit. */
  addOns: ServiceLite[];
  selectedIds: string[];
  ...
  return (
    <Card as="fieldset" className="space-y-2">
      <legend className="px-2 text-sm font-medium">Add-ons (optional)</legend>
        ...
        {addOns.map((s) => {
          ...
          return (
            <label
              key={s.id}
              className={cn(
                "flex items-start gap-3 rounded-xl border p-3 cursor-pointer min-w-0",
