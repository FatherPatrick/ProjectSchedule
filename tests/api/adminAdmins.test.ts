 ...
 * are mocked so we don't go through Prisma in unit tests. All three
 * routes are salon-scoped via `session.user.salonId` — there's no more
 * env-managed admin allow-list (that bootstrap was removed once signup
...
const adminSession = {
  user: { id: "u1", role: "ADMIN" as const, salonId: "salon_1" },
};
...
describe("GET /api/admin/admins", () => {
  ...
  it("returns the admin list for the caller's salon with ISO timestamps", async () => {
    ...
    expect(listAdminPhonesMock).toHaveBeenCalledWith("salon_1");
  });
});
...
describe("POST /api/admin/admins", () => {
  ...
  it("normalises and calls addAdminPhone scoped to the caller's salon", async () => {
    const res = await call({ phone: "5555550001" });
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.phone).toBe("+15555550001");
    expect(addAdminPhoneMock).toHaveBeenCalledWith("salon_1", "+15555550001", "u1");
  });
});
...
describe("DELETE /api/admin/admins/[phone]", () => {
  ...
  it("200s on success, scoped to the caller's salon", async () => {
    prismaMock.user.findUnique.mockResolvedValueOnce({ phone: "+15555559999" });
    removeAdminPhoneMock.mockResolvedValueOnce(true);
    const res = await call("+15555550004");
    expect(res.status).toBe(200);
    expect(removeAdminPhoneMock).toHaveBeenCalledWith("salon_1", "+15555550004");
  });
});
